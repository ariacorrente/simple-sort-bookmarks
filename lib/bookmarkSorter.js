...
function getProperty(itemId) {
    try {
        if (simplePrefs.prefs.sortBy == 1) {
            return String(bmsvc.getBookmarkURI(itemId).spec) || "";
        } else if (simplePrefs.prefs.sortBy == 2) {
            return String(bmsvc.getItemDateAdded(itemId)) || "";
        } else {
            return String(bmsvc.getItemTitle(itemId)) || "";
        }
    } catch (e1) {
        try {
            return String(bmsvc.getItemTitle(itemId)) || "";
...
function getSortedKeys(obj) {
    return Object.keys(obj).sort(function(a, b) { return obj[a].toLowerCase().localeCompare(obj[b].toLowerCase()); });
}
...
*/
function sortItems() {
    var i, node, sepCount = 0, bookmarks = [{}], folders = [{}], order = [];
    alreadyRunning = true;
    var id = folderId.pop();
    ...
    if (isBlacklisted(id)) {
        ...
        for (i = 0; i < rootNode.childCount; i++) {
            ...
            if (node.type === node.RESULT_TYPE_SEPARATOR) {
                ...
                folders[sepCount][node.itemId] = getProperty(node.itemId);
                if (isRecursive) {
                    folderId.push(node.itemId);
                }
            } else {
                bookmarks[sepCount][node.itemId] = getProperty(node.itemId);
            }
        }

        for (i = 0; i <= sepCount; i++) {
            if (simplePrefs.prefs.mustSortFolderFirst || simplePrefs.prefs.sortBy == 1) {
                order = order.concat(getSortedKeys(folders[i]), getSortedKeys(bookmarks[i]));
            } else {
                order = order.concat(getSortedKeys(Object.assign(folders[i], bookmarks[i])));
            }
            order.push(-1);
        }
