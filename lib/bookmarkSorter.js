...
function sortSimple(rootNode, isRecursive) {
    ...
    for (i = 0; i < rootNode.childCount; i++) {
        ...
        //Skip sorting if we are in "Bookmark Menu" and option is set
        if(rootNode.itemId === bmsvc.bookmarksMenuFolder &&
            simplePrefs.prefs.mustSkipRootFolder === true) {
            console.log("Skipping root folder because of user preference");
        } else {
...
An array is required for multiple ids because calling too fast sortFolder will
cause the next execution to skip because of the "alreadyRunning" check.
*/
function sortFolder(newId, sortChild) {
    if (!alreadyRunning) {
        ...
        //Push the newId or the newIds in the queue
        if(Array.isArray(newId)) {
            for(var i = 0; i < newId.length; i++) {
                folderId.push(newId[i]);
            }
        } else {
            folderId.push(newId);
        }
...
/*
Sort recursivly the bookmarks menu, the toolbar, the unfiled bookmarks and the
mobile bookmarks.
...
function sortEverything() {
    var allFolders = [bmsvc.bookmarksMenuFolder,
                      bmsvc.toolbarFolder,
                      bmsvc.unfiledBookmarksFolder,
                      bmsvc.mobileFolder];
    sortFolder(allFolders, true);
}
